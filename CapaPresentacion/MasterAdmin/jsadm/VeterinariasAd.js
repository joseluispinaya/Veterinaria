...
function listaVeterinarias() {
    ...
    table = $("#tbVeterina").DataTable({
        ...
        "columns": [
            ...
            {
                "data": "DocumentoPdf", render: function (data) {
                    if (data) {
                        return `<a href="${data}" target="_blank" class="btn btn-info btn-sm"><i class="fas fa-file-pdf"></i> Ver PDF</a>`;
                    } else {
                        return '<span class="badge badge-warning">Sin PDF</span>';
                    }
                },
                "orderable": false,
                "searchable": false,
            ...
            {
                "data": "OpcionPdf", render: function (data) {
                    let tutorButton = '';
                    if (data === false) {
                    ...
                    return `<button class="btn btn-primary btn-editar btn-sm me-5px" title="Editar"><i class="fas fa-pencil-alt"></i> Editar</button>
                            ${tutorButton}`;
                },
                "orderable": false,
                "searchable": false,
...
$("#tbVeterina tbody").on("click", ".btn-editar", function (e) {
    ...
    // Mostrar el tab "Registro Veterinaria"
...
$("#tbVeterina tbody").on("click", ".btn-agregar-pdf", function (e) {
    e.preventDefault();
    let filaSeleccionada;
    if ($(this).closest("tr").hasClass("child")) {
        filaSeleccionada = $(this).closest("tr").prev();
...
$("#tbVeterina tbody").on("click", ".btn-editar-pdf", function (e) {
    e.preventDefault();
    let filaSeleccionada;
    if ($(this).closest("tr").hasClass("child")) {
        filaSeleccionada = $(this).closest("tr").prev();
...
function registerPdfAjaxP() {
    ...
    if (file) {
        ...
        reader.onload = function (e) {
            ...
            var request = {
                IdVeterinaria: parseInt($("#txtIdVeterinaPdf").val()),
            ...
            $.ajax({
                ...
                dataType: "json",
                beforeSend: function () {
                    $("#loaddpd").LoadingOverlay("show");
                },
                success: function (response) {
                    $("#loaddpd").LoadingOverlay("hide");
                    if (response.d.Estado) {
                        listaVeterinarias();
                        $('#modalpdf').modal('hide');
                        swal("Mensaje", response.d.Mensaje, "success");
...
$('#btnGuardarCambiosPdf').on('click', function () {
    ...
    if (est) {
        ...
        //editarPdfAjaxP();
        swal("Mensaje", "Falta Implementar para actualizar pdf", "warning");
        $('#btnGuardarCambiosPdf').prop('disabled', false);
    }
